...
contract CrossChainTWAPReceiver is LoremIpsumHandler {
    uint32 srcChainId;
    ...
    function handleMessageImpl(uint32 _srcChainId, address sender, bytes memory data)
    internal override {
    ...
    public view returns (uint256 priceX96, uint256 timestamp) {
        bytes32 key = keccak256(abi.encode(poolAddress, twapInterval));
