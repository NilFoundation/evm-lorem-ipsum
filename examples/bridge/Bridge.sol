...
contract Deposit is Ownable {
    ...
    function deposit(address recipient, uint256 amount, address tokenAddress, uint32 chainId) external payable virtual {
...
contract Withdraw is Ownable, LoremIpsumHandler {
    ...
    /// @param _telepathyReceiver the address of the telepathy receiver contract on this chain that can call `handleMessage`
    ...
    LoremIpsumHandler(_telepathyReceiver) {
    ...
    function handleMessageImpl(uint32 _sourceChainId, address _senderAddress, bytes memory _data) internal override {
        require(_senderAddress == depositAddress,
