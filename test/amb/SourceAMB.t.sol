...
contract SourceAMBTest is Test {
    ...
    LoremIpsumRouter wrappedSourceAMBProxy;

    address bob = payable(makeAddr("bob"));

    function setUp() public {
        LoremIpsumRouter sourceAMBImplementation = new LoremIpsumRouter();
        UUPSProxy proxy = new UUPSProxy(address(sourceAMBImplementation), "");

        wrappedSourceAMBProxy = LoremIpsumRouter(address(proxy));
