...
contract SimpleHandler is ILoremIpsumHandler {
...
library WrappedInitialize {
    function init(
        ...
        LoremIpsumRouter(targetAMB).initialize(
...
contract TargetAMBTest is Test {
        ...
        internal
    {
        ...
        LoremIpsumRouter targetAMBImplementation = new LoremIpsumRouter();

        UUPSProxy proxy = new UUPSProxy(address(targetAMBImplementation), "");

        targetAMB = LoremIpsumRouter(address(proxy));
    ...
    function test_ExecuteMessage() public {
        // This test is generated using `cli/contracts/generateTest.ts`
    ...
    function test_RevertExecuteMessage_WhenInvalidDestination() public {
        // Test what happens when the destination address doesn't implement the ILoremIpsumHandler
        ExecuteMessageFromStorageParams memory messageParams = parseParams("storage1");
        getDefaultContractSetup(messageParams);
